...
void set_message_handlers(GstBus *bus, const char* sql_file) {
	...
	//g_signal_connect(pipeline, "have-type", G_CALLBACK(have_type), NULL);
...
void deep_element_stuff(GstBin* self, GstBin* sub_bin, GstElement* element, gpointer data) {
	...
	else if (genaric_regex(gst_object_get_name(GST_OBJECT(element)), "souphttpsrc") == 0) {
...
//plan
// source ! ! autoaudiosink
/*void custom_pipeline_fix() {
	GstElement* bin;
	GstElement* tsparse;
